...
export function Shop() {
  ...
  return <div className="pt-24 pb-16 min-h-screen">
    ...
    className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${selectedCategory === category ? "bg-black text-white" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
  >
                {category}
              </button>)}
          </div>
  ...
  >
    ...
    /* Product Grid */
        ...
        {loading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          ...
          </div> : filteredProducts.length === 0 ? <div className="text-center py-24">
  ...
  />
                  ...
                  {product.stock === 0 && <div className="absolute top-3 left-3 bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded">
                      Sold Out
                    </div>}
    ...
    className={`w-full py-3 rounded-lg font-medium transition-colors shadow-lg ${product.stock === 0 ? "bg-gray-200 text-gray-500 cursor-not-allowed" : "bg-white/90 backdrop-blur-sm text-black hover:bg-black hover:text-white"}`}
  >
